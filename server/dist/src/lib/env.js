...
function requireEnv(name) {
    ...
    if (!value) {
        throw new Error(`${name} environment variable is required. Set it in your .env file locally or in your deployment environment.`);
    }
    return value;
}
...
exports.env = {
    ...
    deliveryFeeInsideCity: parseNonNegativeNumber(process.env.DELIVERY_FEE_INSIDE_CITY, 80),
    deliveryFeeOutsideCity: parseNonNegativeNumber(process.env.DELIVERY_FEE_OUTSIDE_CITY, 150),
    rateLimitWindowMs: parseNonNegativeNumber(process.env.RATE_LIMIT_WINDOW_MS, 60_000),

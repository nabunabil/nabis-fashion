...
const baseURL = env_1.env.betterAuthUrl;
const supportEmail = env_1.env.supportEmail;
const trustedOrigins = Array.from(new Set([
    ...env_1.env.trustedOrigins,
    env_1.env.trustedOrigin,
    env_1.env.betterAuthUrl,
    "http://localhost:3000",
].filter(Boolean)));
const googleClientId = env_1.env.googleClientId;
const googleClientSecret = env_1.env.googleClientSecret;
const useSecureCookies = env_1.env.nodeEnv === "production";
        ...
        _authInstance = betterAuth({
            ...
            emailAndPassword: {
                ...
                sendResetPassword: async ({ user, url }, request) => {
                    ...
                    const token = urlObj.searchParams.get("token") || urlObj.pathname.split("/").pop();
                    const frontendUrl = `${env_1.env.trustedOrigin}/reset-password?token=${token}`;
